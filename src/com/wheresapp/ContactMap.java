    ...
    private void addMarkers() {
        if (map != null) {
            ...
            if (location != null) {
                ...
                map.addMarker(new MarkerOptions().position(fromPosition).title("@string/my_location")
                    .icon(BitmapDescriptorFactory
                    .defaultMarker(BitmapDescriptorFactory.HUE_GREEN)).alpha(0.4f).draggable(true));
            } else {
                Toast.makeText(getApplicationContext(), "Esperando localización", Toast.LENGTH_LONG).show();
            }

            // a draggable marker with title and snippet
            // marker using custom image
            map.addMarker(new MarkerOptions().position(toPosition).title("@string/goal")
